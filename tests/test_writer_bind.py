...
HOSTS_WRITE: Final[list[str]] = [
    'example.com CNAME .', 'x.example.com CNAME .', 'y.example.com CNAME .',
    'example.net CNAME .', '*.example.net CNAME .',
    'example.org CNAME .', 'x.example.org CNAME .',
]
...
def test_write_hosts_list_end_wildcard() -> None:
    with tempfile.TemporaryFile('r+', encoding='utf-8') as file:
        hostblocker.writer.bind.write_hosts_list(['d.com', 'a.d.com', 'b.d.com', 'c.d.com'], file)
        file.seek(0)
        lines = file.readlines()
    assert len(lines) == 2
    assert list(map(str.strip, lines)) == ['d.com CNAME .', '*.d.com CNAME .']
...
def test_write() -> None:
    ...
    assert list(map(str.strip, lines[7:14])) == HOSTS_WRITE
